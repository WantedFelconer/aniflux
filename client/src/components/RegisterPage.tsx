...
export default function RegisterPage({ onNavigate, isModal = false, onSuccess }: RegisterPageProps) {
  const { register, isLoading, authError, clearError, socialLogin } = useAuth()
  ...
  const strengthInfo = useMemo(() => {
    if (!password) return { score: 0, label: '', color: '#282b34' }
    let score = 0
  ...
  const cardContent = (
    <div className="w-full max-w-md mx-auto p-6 sm:p-8 rounded-3xl relative overflow-hidden" style={{ background: '#111216', border: '1px solid #23252b', boxShadow: '0 20px 50px rgba(0,0,0,0.6)' }}>
      ...
      <div className="absolute -top-24 -right-24 w-48 h-48 rounded-full pointer-events-none" style={{ background: '#6d3bff', filter: 'blur(90px)', opacity: 0.25 }} />
      <div className="absolute -bottom-24 -left-24 w-48 h-48 rounded-full pointer-events-none" style={{ background: '#ff4db8', filter: 'blur(90px)', opacity: 0.25 }} />
      ...
      <div className="text-center mb-6 relative z-10">
        ...
        <h2 className="text-2xl font-black tracking-tight text-white">Join Aniflux</h2>
        <p className="text-xs mt-1" style={{ color: '#a0a0a0' }}>Create an account to track, rate, and discover anime</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-3.5 relative z-10">
        {/* Username */}
        <div>
          <label className="block text-xs font-semibold mb-1" style={{ color: '#c0c0c0' }}>Username</label>
        ...
        {/* Email */}
        <div>
          <label className="block text-xs font-semibold mb-1" style={{ color: '#c0c0c0' }}>Email Address</label>
          <div className="relative">
            <input
              type="email"
              required
        ...
        {/* Password */}
        <div>
          <label className="block text-xs font-semibold mb-1" style={{ color: '#c0c0c0' }}>Password</label>
          <div className="relative">
            <input
        ...
        {/* Confirm Password */}
        <div>
          <label className="block text-xs font-semibold mb-1" style={{ color: '#c0c0c0' }}>Confirm Password</label>
          <div className="relative">
            <input
              type={showPassword ? 'text' : 'password'}
              required
        ...
        {/* Terms */}
        <div className="pt-1">
          <label className="flex items-start gap-2.5 cursor-pointer text-xs select-none" style={{ color: '#a0a0a0' }}>
            <input
              type="checkbox"
              checked={agreeTerms}
              onChange={(e) => setAgreeTerms(e.target.checked)}
              className="w-4 h-4 rounded cursor-pointer mt-0.5 accent-[#6d3bff]"
            />
            <span className="leading-snug">
              I agree to the{' '}
              <a href="#terms" onClick={(e) => e.preventDefault()} className="text-white underline">Terms of Service</a> and{' '}
              <a href="#privacy" onClick={(e) => e.preventDefault()} className="text-white underline">Privacy Policy</a>
            </span>
          </label>
        </div>
      ...
      <div className="relative my-5 text-center">
        ...
        <span className="relative px-3 text-xs uppercase tracking-wider font-semibold" style={{ background: '#111216', color: '#6b6b6b' }}>Or sign up with</span>
      </div>
...
function CheckShieldIcon() {
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"><path d="M8 2L3 4v4c0 3.5 2.5 6 5 6s5-2.5 5-6V4l-5-2z"/><path d="M6 8l1.5 1.5L10.5 6.5"/></svg>
}
